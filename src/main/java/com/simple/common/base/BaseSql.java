...
public abstract class BaseSql {
	...
	protected <T, C extends BaseCondition> Page<T> page(boolean logSql, String sql, final C cond, final Class<T> clazz) {
		...
		int offset = (cond.getPage() - INT_1) * cond.getSize();
		int limit = cond.getSize();
		String pageSql = dialect.buildPageSql(sql + cond.where() + cond.orders(), offset, limit);
		List<T> data = list(logSql, pageSql, clazz, cond.array());
