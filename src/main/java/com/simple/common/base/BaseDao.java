...
public class BaseDao<T> extends BaseSql {
    ...
    public <C extends BaseCondition> int update(final T t, final C c) {
        return update(showSql, t, c);
    }

    public <C extends BaseCondition> int update(boolean show, final T t, final C c) {
        FieldUtil.Update update = byCondition(fields, t, c, currentUserId());
        String sql = Sql.builder().update().table(table).set(update.sql()).sql();
        return update(show, sql, update.array);
    }
    ...
    public <C extends BaseCondition> int updateNull(final T t, final C c) {
        return updateNull(showSql, t, c);
    }

    public <C extends BaseCondition> int updateNull(boolean show, final T t, final C c) {
    ...
    public int update(final T t) {
        return update(showSql, t);
    }

    public int update(boolean show, final T t) {
    ...
    public int updateNull(final T t) {
        return updateNull(showSql, t);
    }

    public int updateNull(boolean show, final T t) {
    ...
    public T replace(final T t) {
        return replace(showSql, t);
    }

    public T replace(boolean show, final T t) {
        if (SNOW.equals(idType) || UUID.equals(idType)) {
            if (!FieldUtil.idIsNull(fieldMap, t, idName)) {
                FieldUtil.Update update = byObject(fields, t, currentUserId());
                String sql = Sql.builder().update().table(table).set(update.sql()).sql();
                update(show, sql, update.array);
                return t;
            } else {
                Object id = SNOW.equals(idType) ? SnowflakeId.nextId() : uuid();
                FieldUtil.Insert<T> insert = snowId(fields, t, id, idName, logicDeleteField, currentUserId());
    ...
    public <C extends BaseCondition> boolean exists(final C c) {
        return exists(showSql, c);
    }

    public <C extends BaseCondition> boolean exists(boolean show, final C c) {
        return count(show, c) > INT_0;
    }
    ...
    public <C extends BaseCondition> int count(final C c) {
        return count(showSql, c);
    }

    public <C extends BaseCondition> int count(boolean show, final C c) {
        String sql = Sql.builder().select().count().from().table(table).as().sql();
        return count(show, sql, c);
    }
    ...
    public <C extends BaseCondition> Page<T> page(final C c) {
        return page(showSql, c);
    }

    public <C extends BaseCondition> Page<T> page(boolean show, final C c) {
        String sql = Sql.builder().select().fields(fields).from().table(table).as().sql();
        return page(show, sql, c, clazz);
    }
    ...
    public T findById(final Object id) {
        return findById(showSql, id);
    }

    public T findById(boolean show, final Object id) {
        return findById(show, id, false);
    }
    ...
    public T findById(final Object id, final Boolean lock) {
        return findById(showSql, id, lock);
    }

    public T findById(boolean show, final Object id, final Boolean lock) {
        String sql;
        if (lock) {
        ...
        return list(show, sql, clazz, id).stream().findFirst().orElse(null);
    }
    ...
    public <C extends BaseCondition> T row(final C c) {
        return row(showSql, c);
    }

    public <C extends BaseCondition> T row(boolean show, final C c) {
        String sql = Sql.builder().select().fields(fields).from().table(table).as().where(c).sql();
        return row(show, sql, clazz, c.array());
    }
    ...
    public List<T> replaceBatch(final List<T> list) {
        return replaceBatch(showSql, list);
    }
